  ...
  public int[] compute(int[] input, int width, int height) {
    ...
    for (int y = 0; y < height; ++y) {
      for (int x = 0; x < ((width * STEPSIZE) - 2); x += STEPSIZE) {
        ...
        // if current pixels red value is greater than the average, increase current value, otherwise decrease
        if (red > meanRed) { output[currentIndex] = (int)((red * factor) > 255 ? 255 : (red * factor)); }
        else { output[currentIndex] = (int)(red / factor); }
        
        // if current pixels green value is greater than the average, increase current value, otherwise decrease
        if (green > meanGreen) { output[currentIndex + 1] = (int)((green * factor) > 255 ? 255 : (green * factor)); }
        else { output[currentIndex + 1] = (int)(green / factor); }
        
        // if current pixels blue value is greater than the average, increase current value, otherwise decrease
        if (blue > meanBlue) { output[currentIndex + 2] = (int)((blue * factor) > 255 ? 255 : (blue * factor)); }
